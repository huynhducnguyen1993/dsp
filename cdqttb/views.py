...
class Loadxemthongbao(View):
    def get(self,request):
        idthongbao = request.GET.get('idthongbao')
        csrf = request.GET.get('csrfmiddlewaretoken')
        tb = Thongbao.objects.get(pk=idthongbao)
        
        nhanvien = Nhanvien.objects.get(username = request.user)
        dct = dict()
        dct_ = dict()
        viewers = tb.viewers
        id_viewers = tb.id_viewers
        
        if viewers !=None :
            for key in viewers:
                dct.update({
                    'ten_'+str(viewers[key]):viewers[key],
                    'ten_'+str(nhanvien.tennv):nhanvien.tennv,
                })
        else:
            dct.update({
        ...
        if id_viewers !=None :
            for key in id_viewers:
                dct_.update({
                    'id_'+str(id_viewers[key]):id_viewers[key],
                    'id_'+str(nhanvien.id):nhanvien.id,
                })
        else:
            dct_.update({
        ...
        form = ThongbaoFormedit(query,instance=tb)
        ...
        return render(request,'nhanvien/load_thongbao.html',context)